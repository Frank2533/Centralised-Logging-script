...
import pickle
import loggingvar as variables
...
def handle_client(conn, addr, max_log_size_bytes=100*1024*1024):
        ...
        if addr[0] in Connections["ACTIVE_CONNECTIONS"].keys():
                ...
                Connections["ACTIVE_CONNECTIONS"][addr[0]][len(Connections["ACTIVE_CONNECTIONS"][addr[0]].keys())] = {"Last Contact":str(datetime.datetime.now()), "First Contact":str(datetime.datetime.now())}
        else:
                Connections["ACTIVE_CONNECTIONS"][addr[0]] = {}
                Connections["ACTIVE_CONNECTIONS"][addr[0]][key] = {"Last Contact":str(datetime.datetime.now()), "First Contact":str(datetime.datetime.now())}
        ...
        data2check = variables.fields_to_monitor
        fields = [(x.split('|')[0],x.split('|')[1]) for x in data2check.values()]
        # Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['First Contact'] = str(datetime.datetime.now())

        while connected:
                # logging.basicConfig(filename=f'{addr[0]}.log', level=logging.INFO,
                #                                         format="%(asctime)s - %(levelname)s - %(message)s")
                # Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'] = {}
                if 'info' not in Connections["ACTIVE_CONNECTIONS"][addr[0]][key].keys():
                        Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'] = {}
                try:
                        if msg_length:
                                try:
                                ...
                                if msg!="PING" and msg!=DISCONNECT_MSG:
                                        if "STATS_INFO" in msg:
                                                for field in fields:
                                                        if field[0] in msg:
                                                                if 'int' in field[1]:
                                                                        if field[0] not in Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'].keys():
                                                                                Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'][field[0]]=0
                                                                        Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'][field[0]]+=int(msg.split(':-:')[-1].split(f'{field[0]} -')[1].split('-')[0].strip())
                                                                if 'str' in field[1]:
                                                                        Connections["ACTIVE_CONNECTIONS"][addr[0]][key]['info'][
                                                                                field[0]] = str(
                                                                                msg.split(':-:')[-1].split(f'{field[0]} -')[
                                                                                        1].split('-')[0].strip())
